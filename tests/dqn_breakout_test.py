...
# Network
layers = [FullyConnected(128, "relu"), FullyConnected(128, "relu"), FullyConnected(128, "relu"), FullyConnected(128, "relu"), FullyConnected(action_dim, "linear")]
...
for iter in range(1):
    reward_per_episode, steps_per_episode, loss_per_episode = trainer.train_dqn(episodes=1500, time_steps=10000, C=100, min_replay_count=int(32),
                                                              batch_size=int(32), epsilon_decay=0.99, learning_rate=0.0002, lr_decay = 0.9972,  discount=0.99, epsilon=1, duration = 100000, lr_low=0.00005,
    ...
    if iter == 0:
        with open('dqn_breakout.pkl', 'wb') as output:
            trainer.model.replay_memory = None
            pickle.dump(trainer.model, output, pickle.HIGHEST_PROTOCOL)


#
np.save( "dqn_breakout_rewards", np.array(re))
np.save("dqn_breakout_steps", np.array(se))
np.save("dqn_breakout_loss", np.array(le))
